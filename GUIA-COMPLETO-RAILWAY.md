# 🚀 Guia Completo de Deploy - LinaX no Railway

**Status:** ✅ Pronto para Deploy
**Plataforma:** Railway.app
**Tempo Estimado:** 45-60 minutos por fase
...
- [x] Frontend URL pronto para atualizar
...
4. Autorize Railway para acessar seus repositórios GitHub

### 1.2 Conectar Repositório
1. Railway pedirá permissão para acessar GitHub
...
- Configuração de build: `cd backend && pnpm install && pnpm run build`
- Configuração de start: `cd backend && pnpm run start`
...
Railway criará automaticamente a variável:
...
# Segurança JWT (IMPORTANTE - Gerar novo!)
...
# URL do Frontend (será preenchida após obter domínio Railway)
...
# URLs de Retorno de Pagamento (ajustar após obter domínio)
...
- O `package.json` roda `prisma db push` automaticamente após `pnpm install`
...
Página deve carregar (pode estar com estilo faltando - é normal).

---

## 📱 Passo 8: Atualizar Frontend URL da API (5 min)

### 8.1 Identificar a URL

A URL deve ser:
- **Sem** `http://localhost:3001`
- **Com** o domínio Railway obtido
...
git commit -m "Atualizar URL da API para produção Railway"
git push origin master
```

Railway fará **auto-deploy** automaticamente após o push. Aguarde ~5 min.
...
4. **Copie a chave gerada** (salvar em lugar seguro)

#### Atualizar Railway
1. Railway Dashboard → Service → Variables
2. Cole em `SENDGRID_API_KEY`: Sua chave API
...
3. Clique **"Create"**
4. **Verifique seu email** (confirmar o link)
...
**Solução:** Verificar tamanho de node_modules, otimizar build
...
- Railway CLI: `npm i -g @railway/cli` (gerenciar via terminal)
