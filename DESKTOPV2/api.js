...
const api = {
  ...
  getDpEspelho: (inicio, fim) => {
    ...
    return fetchApi(`/dp/espelho?${params}`);
  },
  ...
  // Afastamentos
  getDpAfastamentos: (funcionarioId) => fetchApi(`/dp/afastamentos${funcionarioId ? `?funcionarioId=${funcionarioId}` : ''}`),
  criarDpAfastamento: (data) => fetchApi('/dp/afastamentos', { method: 'POST', body: JSON.stringify(data) }),
  ...
  deleteAdiantamento:   (id) => fetchApi(`/caixa/adiantamento/${id}`, { method: 'DELETE' }),
  ...
  emailAutomacao: {
    ...
    criarDestinatario: (dados)  => fetchApi('/email-automacao/destinatarios', { method: 'POST', body: JSON.stringify(dados), semRedirect: true }),
    removerDestinatario: (id)   => fetchApi(`/email-automacao/destinatarios/${id}`, { method: 'DELETE', semRedirect: true }),
